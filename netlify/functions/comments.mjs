...
function isValidAdminPassword(providedPassword) {
  const savedPassword = process.env.WEDDING_ADMIN_PASSWORD || "";
  const provided = String(providedPassword ?? "");

  if (!savedPassword || !provided) return false;

  const savedBuffer = Buffer.from(savedPassword, "utf8");
  const providedBuffer = Buffer.from(provided, "utf8");

  if (savedBuffer.length !== providedBuffer.length) return false;
  return timingSafeEqual(savedBuffer, providedBuffer);
}
...
export default async (request) => {
  ...
  if (request.method === "PUT") {
    try {
      if (!process.env.WEDDING_ADMIN_PASSWORD) {
        return json({ error: "관리자 비밀번호가 아직 설정되지 않았습니다." }, 503);
      }
      ...
      return json({ error: "관리자 인증 중 오류가 발생했습니다." }, 500);
    }
  }
  ...
  if (request.method === "DELETE") {
    try {
      ...
      if (!/^[0-9a-f-]{36}$/i.test(id)) {
        return json({ error: "삭제할 메시지 정보가 올바르지 않습니다." }, 400);
      }

      const { blobs } = await store.list({ prefix: "comment/" });
      const target = blobs.find(({ key }) => key.endsWith(`-${id}`));

      if (!target) {
        return json({ error: "이미 삭제되었거나 존재하지 않는 메시지입니다." }, 404);
      }
      ...
      return json({ error: "메시지를 삭제하지 못했습니다." }, 500);
    }
  }
